-- Account deletion may null nullable actor references through FK cascades. Keep
-- invoice content immutable while allowing only this nested metadata cleanup.
...
begin
  if pg_trigger_depth() > 1
     and (to_jsonb(old) - array['created_by','updated_by','updated_at','version'])
         is not distinct from
         (to_jsonb(new) - array['created_by','updated_by','updated_at','version']) then
    return new;
  end if;
