...
export function InvoiceDocument({ invoice, customer, company }: { invoice: Invoice; customer?: Customer; company: Company }) {
  ...
  return <article id="invoice-print-root" className="invoice-preview-page invoice-document">
    ...
    <header className="preview-head"><img src="/logo-rechnung.jpg" alt="Photographie Blitzidee" /><div><strong>{company.name}</strong><span>{company.owner}</span><span>{company.street}, {company.postalCode} {company.city}</span><span>{company.email}</span><a href={company.website} target="_blank" rel="noopener noreferrer">photographie-blitzidee.de</a></div></header>
