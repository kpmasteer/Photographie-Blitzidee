...
import { calculateDiscount, itemTotal, euro, openAmount, parseEuroToCents, parsePriceInput } from "../lib/money";
import { InvoicePreview } from "../components/InvoicePreview";
import { createInvoicePdf, downloadBlob } from "../lib/pdf";
...
export function InvoiceEditor() {
  ...
  const [priceInputs, setPriceInputs] = useState<Record<string, string>>({}); const [itemErrors, setItemErrors] = useState<Record<string, string[]>>({}); const [showPreview, setShowPreview] = useState(false);
  ...
  useEffect(() => { setForm((current) => { if (!current) return current; let changed = false; const items = current.items.map((item) => { const raw = priceInputs[item.id]; if (raw === undefined) return item; const parsed = parsePriceInput(raw); if (!parsed.valid || parsed.cents === item.unitPriceCents) return item; changed = true; return { ...item, unitPriceCents: parsed.cents, totalCents: itemTotal(item.quantityMilli, parsed.cents) }; }); return changed ? { ...current, items, totalCents: items.reduce((sum, item) => sum + item.totalCents, 0), updatedAt: new Date().toISOString() } : current; }); }, [priceInputs]);
  ...
  const recalc = (items: InvoiceItem[]) => { const subtotalCents = items.reduce((sum, item) => sum + item.totalCents, 0); setForm((current) => { if (!current) return current; const discounted = calculateDiscount(subtotalCents, current.discountType, current.discountValue || 0); return { ...current, items, subtotalCents, discountCents: discounted.discountCents, totalCents: discounted.totalCents, updatedAt: new Date().toISOString() }; }); };
  const patchItem = (id: string, patch: Partial<InvoiceItem>) => recalc(form!.items.map((item) => {
    if (item.id !== id) return item; const next = { ...item, ...patch }; const subtotalCents = itemTotal(next.quantityMilli, next.unitPriceCents); const discounted = calculateDiscount(subtotalCents, next.discountType, next.discountValue || 0); return { ...next, subtotalCents, discountCents: discounted.discountCents, totalCents: discounted.totalCents };
  }));
  ...
  const print = () => { setShowPreview(true); window.setTimeout(() => window.print(), 100); };
  ...
  return <>
    ...
    <div className="editor-grid"><section className="panel editor-form"><h2>Rechnungsdaten</h2><div className="form-grid">
      ...
      {editable && <div className="position-discounts"><h3>Positionsrabatte</h3>{form.items.map((item, index) => <div className="discount-controls" key={item.id}><span>Position {index + 1}</span><label>Rabattart<select value={item.discountType || ""} onChange={(e) => patchItem(item.id, { discountType: (e.target.value || undefined) as InvoiceItem["discountType"], discountValue: 0 })}><option value="">Kein Rabatt</option><option value="percent">Prozent</option><option value="fixed">Fester Betrag</option></select></label>{item.discountType && <label>{item.discountType === "percent" ? "Rabatt in %" : "Rabattbetrag"}<input inputMode="decimal" value={item.discountValue || ""} onFocus={(e) => e.currentTarget.select()} onChange={(e) => patchItem(item.id, { discountValue: item.discountType === "fixed" ? parseEuroToCents(e.target.value) : Number(e.target.value.replace(",", ".")) })} /></label>}{Boolean(item.discountCents) && <span>−{euro(item.discountCents || 0)} · Position {euro(item.totalCents)}</span>}</div>)}</div>}
      {editable && <div className="discount-controls"><label>Gesamtrabatt<select value={form.discountType || ""} onChange={(e) => { const type = e.target.value as Invoice["discountType"]; update("discountType", type || undefined); window.setTimeout(() => recalc(form.items), 0); }}><option value="">Kein Rabatt</option><option value="percent">Prozent</option><option value="fixed">Fester Betrag</option></select></label>{form.discountType && <label>{form.discountType === "percent" ? "Rabatt in %" : "Rabattbetrag"}<input inputMode="decimal" value={form.discountValue || ""} onFocus={(e) => e.currentTarget.select()} onChange={(e) => { const value = form.discountType === "fixed" ? parseEuroToCents(e.target.value) : Number(e.target.value.replace(",", ".")); update("discountValue", value); window.setTimeout(() => recalc(form.items), 0); }} /></label>}</div>}
